use crate::process::{run_pear_git, UiError, UiErrorCode};
use crate::validation::{validate_branch, validate_pear_url, validate_pubkey, validate_repo_path};
...
pub struct SettingsPayload {
  #[serde(rename = "PEAR_GIT_DATA_DIR")]
  pub data_dir: String,
  #[serde(rename = "PEAR_GIT_LOG_LEVEL")]
  pub log_level: String,
  #[serde(rename = "PEAR_GIT_BOOTSTRAP_NODES")]
  pub bootstrap_nodes: String,
  #[serde(rename = "PEAR_GIT_SEEDER_KEYS")]
  pub seeder_keys: String,
  #[serde(rename = "PEAR_GIT_CONNECT_TIMEOUT")]
...
pub(crate) fn runtime_state() -> &'static Mutex<RuntimeState> {
  RUNTIME_STATE.get_or_init(|| {
    ...
    settings.insert(
      "PEAR_GIT_DATA_DIR".to_string(),
      std::env::var("PEAR_GIT_DATA_DIR").unwrap_or_else(|_| "~/.pear-git".to_string()),
    );
    settings.insert(
      "PEAR_GIT_LOG_LEVEL".to_string(),
      std::env::var("PEAR_GIT_LOG_LEVEL").unwrap_or_else(|_| "info".to_string()),
    );
    settings.insert(
      "PEAR_GIT_BOOTSTRAP_NODES".to_string(),
      std::env::var("PEAR_GIT_BOOTSTRAP_NODES").unwrap_or_default(),
    );
    settings.insert(
      "PEAR_GIT_SEEDER_KEYS".to_string(),
      std::env::var("PEAR_GIT_SEEDER_KEYS").unwrap_or_default(),
    );
    settings.insert(
      "PEAR_GIT_CONNECT_TIMEOUT".to_string(),
      std::env::var("PEAR_GIT_CONNECT_TIMEOUT").unwrap_or_else(|_| "10000".to_string()),
    );
...
pub fn repo_status(path: String) -> Result<RepoStatusResponse, UiError> {
  ...
  let output = run_pear_git(&["status", "--json"], Some(repo_path.as_path()))?;

  let first_line = output
    .stdout
    .lines()
    .find(|line| !line.trim().is_empty())
    .ok_or_else(|| UiError::new(UiErrorCode::CommandFailed, "Missing status output from pear-git"))?;
...
pub fn repo_init(path: String) -> Result<RepoInitResponse, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["init"], Some(repo_path.as_path()))?;

  let url = output
    .stdout
    .lines()
    .map(str::trim)
    .find(|line| line.starts_with("pear://"))
    .ok_or_else(|| UiError::new(UiErrorCode::CommandFailed, "Missing pear:// URL from init output"))?
    .to_string();
...
pub fn repo_clone(url: String, path: String) -> Result<RepoCloneResponse, UiError> {
  let clean_url = validate_pear_url(&url)?;
  let repo_path = validate_repo_path(&path)?;

  run_pear_git(&["clone", clean_url.as_str(), repo_path.to_string_lossy().as_ref()], None)?;
...
pub fn repo_pull(path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["pull"], Some(repo_path.as_path()))?;
...
pub fn repo_push(path: String, branch: String) -> Result<SyncSummary, UiError> {
  ...
  let output = match run_pear_git(&["push", "origin", clean_branch.as_str()], Some(repo_path.as_path())) {
    Ok(value) => value,
    Err(error) => {
      let details = error.details.clone().unwrap_or_default().to_lowercase();
...
pub fn writer_invite(path: String, pubkey: String, indexer: bool) -> Result<SyncSummary, UiError> {
  ...
  let output = run_pear_git(args.as_slice(), Some(repo_path.as_path())).map_err(map_permission_error)?;
...
pub fn writer_revoke(path: String, pubkey: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let clean_pubkey = validate_pubkey(&pubkey)?;
  let output = run_pear_git(&["revoke", clean_pubkey.as_str()], Some(repo_path.as_path())).map_err(map_permission_error)?;
...
pub fn secrets_list(path: String) -> Result<Vec<SecretListItem>, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["secrets", "list", "--json"], Some(repo_path.as_path()))?;
...
pub fn secrets_add(path: String, file_path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["secrets", "add", file_path.as_str()], Some(repo_path.as_path())).map_err(|error| {
    UiError::with_details(
      error.code,
      error.message,
...
pub fn secrets_get(path: String, secret_path: String) -> Result<String, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["secrets", "get", secret_path.as_str()], Some(repo_path.as_path())).map_err(|error| {
    UiError::with_details(
      error.code,
      error.message,
      redact_sensitive_details(error.details.unwrap_or_default().as_str()),
...
pub fn secrets_remove(path: String, secret_path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["secrets", "rm", secret_path.as_str()], Some(repo_path.as_path())).map_err(|error| {
    UiError::with_details(
      error.code,
      error.message,
      redact_sensitive_details(error.details.unwrap_or_default().as_str()),
...
pub fn secrets_rotate(path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let output = run_pear_git(&["secrets", "rotate"], Some(repo_path.as_path())).map_err(|error| {
    UiError::with_details(
      error.code,
      error.message,
      redact_sensitive_details(error.details.unwrap_or_default().as_str()),
...
pub fn seed_start(path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let _ = run_pear_git(&["seed", "start"], Some(repo_path.as_path()));
...
pub fn seed_stop(path: String) -> Result<SyncSummary, UiError> {
  let repo_path = validate_repo_path(&path)?;
  let _ = run_pear_git(&["seed", "stop"], Some(repo_path.as_path()));
...
pub fn settings_get() -> Result<SettingsPayload, UiError> {
  ...
  Ok(SettingsPayload {
    data_dir: guard.settings.get("PEAR_GIT_DATA_DIR").cloned().unwrap_or_default(),
    log_level: guard.settings.get("PEAR_GIT_LOG_LEVEL").cloned().unwrap_or_else(|| "info".to_string()),
    bootstrap_nodes: guard.settings.get("PEAR_GIT_BOOTSTRAP_NODES").cloned().unwrap_or_default(),
    seeder_keys: guard.settings.get("PEAR_GIT_SEEDER_KEYS").cloned().unwrap_or_default(),
    connect_timeout: guard.settings.get("PEAR_GIT_CONNECT_TIMEOUT").cloned().unwrap_or_else(|| "10000".to_string()),
  })
}
...
pub fn settings_set(key: String, value: String) -> Result<SyncSummary, UiError> {
  let allowed = [
    ...
    "PEAR_GIT_CONNECT_TIMEOUT",
  ];

  if !allowed.contains(&key.as_str()) {
    return Err(UiError::new(UiErrorCode::InvalidInput, "Unsupported PEAR_GIT_* setting"));
  }
...
pub fn set_log_level(value: String) -> Result<SyncSummary, UiError> {
  settings_set("PEAR_GIT_LOG_LEVEL".to_string(), value)
}
...
mod tests {
  ...
  fn settings_set_accepts_supported_keys() {
    let result = settings_set("PEAR_GIT_LOG_LEVEL".to_string(), "debug".to_string());
    assert!(result.is_ok());
  }
}
