...
// object-put codec — uploads a git object via an Autobase op. The object
// bytes include the canonical git header so downstream code can read them
// without additional metadata. This keeps object storage inside the Autobase
// view (a.k.a. view.sub('objects')), removing the need for a separate
// multi-writer Hypercore.
export const objectPutCodec = {
  preencode (state, val) {
    c.string.preencode(state, val.op)
    sha40.preencode(state, Buffer.from(val.sha, 'ascii'))
    c.buffer.preencode(state, val.bytes)
    c.fixed64.preencode(state, val.signature)
  },
  encode (state, val) {
    c.string.encode(state, val.op)
    sha40.encode(state, Buffer.from(val.sha, 'ascii'))
    c.buffer.encode(state, val.bytes)
    c.fixed64.encode(state, val.signature)
  },
  decode (state) {
    return {
      op: c.string.decode(state),
      sha: sha40.decode(state).toString('ascii'),
      bytes: c.buffer.decode(state),
      signature: c.fixed64.decode(state)
    }
  }
}

// secret-put codec — stores an encrypted secret file in the Autobase
// view.sub('secret-files') bee so it replicates to all peers. The bytes
// payload includes the [keyVersion(4)|nonce(24)|ciphertext(...)] layout
// the secrets commands already produce.
export const secretPutCodec = {
  preencode (state, val) {
    c.string.preencode(state, val.op)
    c.string.preencode(state, val.path)
    c.buffer.preencode(state, val.bytes)
    c.fixed64.preencode(state, val.signature)
  },
  encode (state, val) {
    c.string.encode(state, val.op)
    c.string.encode(state, val.path)
    c.buffer.encode(state, val.bytes)
    c.fixed64.encode(state, val.signature)
  },
  decode (state) {
...
// secret-del codec — removes a secret from the shared view.
export const secretDelCodec = {
  preencode (state, val) {
    c.string.preencode(state, val.op)
    c.string.preencode(state, val.path)
    c.fixed64.preencode(state, val.signature)
...
// Op type tag mapping — APPEND ONLY to preserve wire compatibility. The
// uint8 tag is an index into this array, so changing order or removing
// entries would corrupt existing Autobase logs.
const OP_TYPES = [
  'ref-update',
  'add-writer',
  'remove-writer',
