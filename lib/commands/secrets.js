...
function validatePath (p, stderr) {
  if (!PATH_RE.test(p) || p.includes('..') || p.length > 255) {
    stderr.write(`gittorrent: error: invalid secret path: ${p}\n`)
    return false
  }
  return true
}
...
async function requireIndexer (repo, identity, stderr) {
  ...
  if (!caller || !caller.indexer) {
    stderr.write('gittorrent: error: only indexers can manage secrets. ' +
      'Ask an existing indexer to run: gittorrent invite <your-pubkey> --indexer\n')
...
export async function runAdd (args, opts) {
  ...
  if (keyVersion === 0) {
    ...
    if (!secretsKey) {
      stderr.write('gittorrent: error: no secrets key available\n')
...
export async function runGet (args, opts) {
  ...
  if (!bytes) {
    stderr.write(`gittorrent: error: secret not found: ${storePath}\n`)
  ...
  if (fileKeyVersion !== currentVersion) {
    stderr.write('gittorrent: error: key version mismatch — rotation in progress, retry shortly\n')
...
export async function runRotate (args, opts) {
  ...
  const caller = writers.find(w => w.key.equals(identity.publicKey))
  if (!caller || !caller.indexer) {
    stderr.write('gittorrent: error: not an indexer — cannot rotate secrets key\n')
    throw new CliError('not an indexer', 2)
  }

  const currentVersion = await getKeyVersion(repo)
  if (currentVersion === 0) {
    stderr.write('gittorrent: error: no secrets key exists yet\n')
    throw new CliError('no secrets key', 2)
  }
