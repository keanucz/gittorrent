...
async function gatherStatus (repo, swarm) {
  ...
  if (typeof repo.getStatusSnapshot === 'function' && !repo.view) {
    ...
    // Best-effort writer list via RPC for UIs that need per-writer keys.
    let writerList = []
    if (typeof repo.getWriters === 'function') {
      try {
        const ws = await repo.getWriters()
        writerList = ws.map(w => ({
          key: Buffer.isBuffer(w.key) ? w.key.toString('hex') : String(w.key),
          indexer: !!w.indexer
        }))
      } catch {}
    }
...
function writeHuman (status, output) {
  ...
  output.write(`Repo: gittorrent://${repoKey}\n`)
...
/**
 * Implement `gittorrent status [--json]`.
 *
 * @param {string[]} args
 * @param {object} opts
...
export async function run (args, opts = {}) {
  ...
  if (repo == null) {
    throw new CliError('not a gittorrent repository', 1)
  }
  ...
  // Non-zero peer count is informational only — never fatal. UIs
  // (tauri app, scripts) need to be able to call `status --json` even
  // when offline. Emit a warning to stderr but keep exit 0 for --json.
  if (status.peers === 0 && !useJson) {
    stderr.write('gittorrent: no peers connected\n')
  }
}
