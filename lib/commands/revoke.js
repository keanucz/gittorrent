...
const ROTATION_WARNING =
  ...
  "Run 'gittorrent secrets rotate' to revoke their access.\n"
...
function ensureNotLastIndexer (writers, target, stderr) {
  ...
  stderr.write('gittorrent: error: cannot remove last indexer\n')
  throw new CliError('cannot remove last indexer', 2)
}

/**
 * Implement `gittorrent revoke <pubkey>`.
 *
 * @param {string[]} args
 * @param {object} opts
...
export async function run (args, opts) {
  ...
  if (!caller || !caller.indexer) {
    stderr.write('gittorrent: error: not an indexer — cannot revoke writers\n')
    throw new CliError('not an indexer — cannot revoke writers', 2)
  }

  const target = writers.find(w => w.key.equals(targetBuf))
  if (!target) {
    stderr.write(`gittorrent: error: ${shortKey(targetBuf)} is not a writer\n`)
