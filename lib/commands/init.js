...
/**
 * Initialize a new gittorrent repo in `cwd`, storing Autobase data under `dataDir`.
 *
...
export async function initRepo ({ cwd, dataDir, name }) {
  ...
  const url = 'gittorrent://' + bs58.encode(Buffer.from(key))
...
/**
 * CLI entry used by `bin/gittorrent init`. Resolves defaults from env, prints
...
export async function run (args) {
  ...
  const dataDir = process.env.GITTORRENT_DATA_DIR || join(homedir(), '.gittorrent')
  ...
  try {
    ...
    process.stderr.write('gittorrent: error: ' + message + '\n')
    process.exit(1)
  }
}
...
async function ensureNotAlreadyInitialised (cwd) {
  ...
  if (existing && existing.startsWith('gittorrent://')) {
    throw new Error(`already a gittorrent repo (origin is ${existing})`)
  }
}
...
async function createRepoStore (dataDir, identity, head) {
  ...
  try {
    ...
    if (err.code === 'ENOTEMPTY' || err.code === 'EEXIST') {
      await rm(finalStorePath, { recursive: true, force: true })
      await rename(tmpStorePath, finalStorePath)
    } else {
      throw err
    }
