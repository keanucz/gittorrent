 ...
 * @param {string[]} args - CLI arguments (repo URLs)
 * @param {object} opts - Options
 * @param {string} [opts.dataDir] - Path to data directory
 * @param {object} [opts.swarm] - Injected swarm manager for testing
 * @param {AbortSignal} [opts.signal] - Signal to stop seeding
 * @param {WritableStream} [opts.output] - Stream to write events to (default stdout)
 */
export async function run (args, opts = {}) {
  ...
  if (allUrls.length === 0) {
    ...
    throw new Error('no repos to seed')
  }
