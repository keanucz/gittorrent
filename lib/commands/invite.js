...
const SECRETS_WARNING =
  ...
  "New writer will not be able to read secrets until an indexer runs 'gittorrent secrets rotate'.\n"
...
async function distributeSecretsKey ({ repo, identity, targetBuf, getMySecretsKeyFn, sealKeyFn, stderr }) {
  // Prefer the RPC-friendly repo API; fall back to direct bee access for the
  // legacy path used by unit tests.
  let currentVersion = 0
  if (typeof repo.getSecretsKeyVersion === 'function') {
    currentVersion = await repo.getSecretsKeyVersion()
  } else if (repo.secretsView) {
    const versionEntry = await repo.secretsView.get('secrets-key-version')
    currentVersion = versionEntry?.value ?? 0
  }
  if (currentVersion === 0) return

  const secretsKey = await getMySecretsKeyFn(repo, identity)
  if (!secretsKey) {
...
/**
 * Implement `gittorrent invite <pubkey> [--indexer]`.
 *
 * @param {string[]} args
 * @param {object} opts
...
export async function run (args, opts) {
  ...
  if (!caller || !caller.indexer) {
    stderr.write('gittorrent: error: not an indexer — cannot invite writers\n')
    throw new CliError('not an indexer — cannot invite writers', 2)
  }

  const existing = writers.find(w => w.key.equals(targetBuf))
  if (existing) {
    stderr.write(`gittorrent: error: ${shortKey(targetBuf)} is already a writer\n`)
