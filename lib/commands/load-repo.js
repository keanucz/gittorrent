...
/**
 * Resolve the gittorrent:// repo URL from the current git working tree by reading
 * the `origin` remote URL.
 *
 * @param {string} cwd
 * @returns {Promise<string>} the gittorrent:// URL
 */
async function readOriginUrl (cwd) {
  const { stdout } = await execFileAsync('git', ['remote', 'get-url', 'origin'], { cwd })
  const url = stdout.trim()
  if (!url.startsWith('gittorrent://')) {
    throw new Error(`origin is not a gittorrent remote: ${url}`)
  }
  return url
}

/**
 * Decode a gittorrent:// URL into the 32-byte repo key.
 *
 * @param {string} url
 * @returns {Buffer}
 */
function decodeRepoKey (url) {
  const encoded = url.replace(/^gittorrent:\/\//, '')
 ...
 */
/**
 * Spawn `gittorrent seed -d` in the background. Returns when a seeder RPC
 * socket for `key` becomes available, or times out.
 */
async function autoStartSeeder (key, timeoutMs = 5000) {
  // Locate the gittorrent binary. We're in lib/commands/load-repo.js so the
  // bin lives at ../../bin/gittorrent relative to this file.
  const here = fileURLToPath(import.meta.url)
  const gittorrentBin = join(here, '..', '..', '..', 'bin', 'gittorrent')

  const child = spawn(process.execPath, [gittorrentBin, 'seed'], {
    detached: true,
    stdio: 'ignore',
    env: process.env
  })
...
export async function loadRepoForCwd (opts = {}) {
  ...
  const autoSpawn = opts.autoSpawnSeeder !== false && process.env.GITTORRENT_NO_AUTO_SEED !== '1'
  ...
  try {
    ...
    if (err && err.code === 'ELOCKED') {
      ...
      if (!rpcClient && autoSpawn) {
        process.stderr.write('gittorrent: store is locked; starting background seeder...\n')
