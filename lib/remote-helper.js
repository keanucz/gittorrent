 ...
 */
async function waitForRefs (repo, timeoutMs, progressFn) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (progressFn && ticks % 10 === 0) {
      progressFn(`Waiting for refs... ${((Date.now() - start) / 1000).toFixed(1)}s`)
    }
  ...
  const helper = {
    ...
    async list () {
      try {
        ...
        if (!refsReady) {
          userProgress('Waiting for refs from peer...')
          await waitForRefs(repo, readyTimeoutMs, userProgress)
          refsReady = true
