...
export async function createRemoteHelper (opts) {
  ...
  if (process.env.PEAR_GIT_STDIN_DIAG !== 'off') {
    input.on('data', (chunk) => {
      const preview = chunk.toString().replace(/\n/g, '\\n')
      writeStderrSync(`pear-git-stdin: <${preview}>\n`)
    })
    input.on('end', () => writeStderrSync('pear-git-stdin: <EOF>\n'))
    input.on('error', (err) => writeStderrSync(`pear-git-stdin: <ERROR ${err.message}>\n`))
  }

  const rl = createInterface({ input, crlfDelay: Infinity })

  const userProgress = (msg) => writeStderrSync(`pear-git: ${msg}\n`)
  ...
  for await (const line of rl) {
    if (process.env.PEAR_GIT_STDIN_DIAG !== 'off') {
      writeStderrSync(`pear-git-rl: [${line}]\n`)
    }
