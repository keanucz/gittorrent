...
// Sync stderr writer — on Windows pipes, node buffers stderr too, so progress
// messages only appear in bursts. Sync write ensures each line reaches the
// terminal immediately.
function writeStderrSync (msg) {
  const buf = Buffer.from(msg)
  try { writeSync(2, buf) } catch { /* best effort */ }
}
...
/**
 * Returns [{ key, value }] for all git refs in the repo.  Uses listRefs()
 * when the repo exposes it (newer repos and RPC proxies); falls back to
 * iterating the view directly otherwise (older tests, legacy paths).
 */
async function readRefs (repo) {
  if (typeof repo.listRefs === 'function') {
    return repo.listRefs()
  }
  const out = []
  for await (const entry of repo.view.createReadStream()) {
    if (isRefKey(entry.key)) out.push({ key: entry.key, value: entry.value })
  }
  return out
}
...
export async function createRemoteHelper (opts) {
  const { input, output, repo, objectStore, workingClonePath, identity, swarm, repoKey, corestore } = opts
  const readyTimeoutMs = opts.readyTimeoutMs ?? 10000
  // Callers can read this after the helper exits to decide whether to
  // auto-seed the clone target. True if git performed any fetch or push
  // during the session (i.e. not a bare `ls-remote`).
  const sessionState = opts.sessionState || {}

  // Diagnostic: log raw stdin chunks so we can tell whether git is actually
  // sending the capabilities/list commands over the pipe on Windows.
  if (process.env.GITTORRENT_STDIN_DIAG !== 'off') {
    input.on('data', (chunk) => {
      const preview = chunk.toString().replace(/\n/g, '\\n')
      writeStderrSync(`gittorrent-stdin: <${preview}>\n`)
    })
    input.on('end', () => writeStderrSync('gittorrent-stdin: <EOF>\n'))
    input.on('error', (err) => writeStderrSync(`gittorrent-stdin: <ERROR ${err.message}>\n`))
  }

  const rl = createInterface({ input, crlfDelay: Infinity })

  const userProgress = (msg) => writeStderrSync(`gittorrent: ${msg}\n`)

  // Track real replication progress (hypercore block downloads across all
  // cores in the store) so the user can see actual sync activity instead of
  // staring at "Waiting for refs..." indefinitely. This surfaces the fact
  // that bytes ARE flowing even before the refs view is fully built.
  let blocksDownloaded = 0
  let lastProgressAt = 0
  if (corestore && typeof corestore.on === 'function') {
    const attachCore = (core) => {
      core.on('download', () => {
        ...
        if (now - lastProgressAt > 500) {
          lastProgressAt = now
          userProgress(`Sync progress: ${blocksDownloaded} blocks received`)
        }
      })
    }
  ...
  for await (const line of rl) {
    if (process.env.GITTORRENT_STDIN_DIAG !== 'off') {
      writeStderrSync(`gittorrent-rl: [${line}]\n`)
    }
...
const OBJECT_WAIT_TIMEOUT_MS = Number(process.env.GITTORRENT_OBJECT_TIMEOUT ?? 8000)
