...
function rpcDir (repoKey) {
  return join(tmpdir(), 'gittorrent-rpc', bs58.encode(repoKey))
}
...
function rpcSocketPath (repoKey, pid) {
  if (isWindows()) {
    return `\\\\.\\pipe\\gittorrent-rpc-${bs58.encode(repoKey).slice(0, 16)}-${pid}`
  }
...
export function listRepoRpcSockets (repoKey) {
  if (isWindows()) {
    ...
    try {
      const dir = join(tmpdir(), 'gittorrent-rpc-registry', bs58.encode(repoKey))
