...
export async function getMySecretsKey (autobaseView, identity) {
  const key = 'secrets-key/' + identity.publicKey.toString('hex')
  const entry = await autobaseView.get(key)
  if (!entry) {
    log.debug({ publicKeyHex: identity.publicKey.toString('hex') }, 'no secrets key envelope found')
    return null
  }
  const encryptedKey = Buffer.from(entry.value.encryptedKey, 'hex')
  const opened = identity.openKeyEnvelope(encryptedKey)
  if (!opened) {
    log.warn({ publicKeyHex: identity.publicKey.toString('hex') }, 'failed to open secrets key envelope')
    return null
  }
