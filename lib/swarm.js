...
// well-known directory. Subprocesses (e.g. `git-remote-gittorrent` launched by git)
...
function repoSocketsDir (repoKey) {
  return join(tmpdir(), 'gittorrent-sockets', bs58.encode(repoKey))
}
...
function startLocalReplicationServer (corestore, repoKey) {
  // Unix-domain sockets don't exist on Windows — `net.listen(path)` hangs
  // silently for a path that isn't the `\\.\pipe\<name>` format. This
  // feature is only used by the e2e test harness to piggyback on a parent
  // process's corestore; real users don't need it. Skip on Windows.
  if (process.platform === 'win32') {
    return Promise.resolve({ server: null, sockPath: null })
  }
...
async function createRelayDHT () {
  const relayUrl = process.env.GITTORRENT_RELAY || DEFAULT_RELAY
  ...
  if (process.env.GITTORRENT_RELAY_TRAFFIC !== 'off') {
    let lastRx = 0
    let lastTx = 0
...
function instrumentConnection (conn, peerIdShort, repoKey, corestore) {
  const diagEnabled = process.env.GITTORRENT_CONN_DIAG !== 'off'
  ...
  const attachCore = (core) => {
    ...
    if (process.env.GITTORRENT_LOG_LEVEL === 'debug') {
...
export async function createSwarm (corestore, opts = {}) {
  ...
  const relayOff = process.env.GITTORRENT_RELAY === 'off' || process.env.GITTORRENT_RELAY === 'none'
  let dht = null
  if (!bootstrap && !opts.bootstrap && !relayOff) {
    try {
      dht = await createRelayDHT()
      log.info({ relay: process.env.GITTORRENT_RELAY || DEFAULT_RELAY }, 'using relay DHT')
    } catch (err) {
  ...
  const connectTimeout = Number(process.env.GITTORRENT_CONNECT_TIMEOUT ?? 10000)
...
async function closeLocalSocket ({ server, sockPath }) {
  if (!server) return // no-op when skipped (e.g. Windows)
  try {
