...
function signableBytes (op) {
  ...
  if (op.path !== undefined) fields.path = op.path
...
async function apply (nodes, view, host) {
  ...
  const secretFilesView = view.sub('secret-files', { valueEncoding: 'binary' })
  ...
  for (const node of nodes) {
    ...
    switch (op.op) {
      ...
      case 'object-put':
        ...
        break
      case 'secret-put':
        await applySecretPut(op, writerKey, host, secretFilesView)
        break
      case 'secret-del':
        await applySecretDel(op, writerKey, host, secretFilesView)
        break
      default:
...
async function applySecretPut (op, writerKey, host, secretFilesView) {
  // Only indexers (known trusted writers) may add secrets. Non-indexer
  // writers can clone but not tamper with the shared encrypted file set.
  if (!isIndexerKey(host, writerKey)) {
    log.warn({ writerKey: writerKey.toString('hex'), path: op.path }, 'secret-put: sender is not an indexer, dropping')
    return
  }
  await secretFilesView.put(op.path, op.bytes)
  log.info({ path: op.path, bytes: op.bytes.length }, 'secret stored')
}

async function applySecretDel (op, writerKey, host, secretFilesView) {
  if (!isIndexerKey(host, writerKey)) {
    log.warn({ writerKey: writerKey.toString('hex'), path: op.path }, 'secret-del: sender is not an indexer, dropping')
    return
  }
  await secretFilesView.del(op.path)
  log.info({ path: op.path }, 'secret deleted')
}
...
export async function openRepo (corestore, identity, opts = {}) {
  ...
  const secretFilesView = base.view.sub('secret-files', { valueEncoding: 'binary' })

  // On a fresh clone (opts.key provided but no local data yet) base.update()
  // will block forever waiting for network peers. Skip it here — callers
  // invoke repo.update() on demand after the swarm is joined.
  const isFreshClone = opts.key && base.local.length === 0 && base._primaryBootstrap?.length === 0
  if (!isFreshClone) {
    await base.update()
  }

  log.info({ repoKey: bs58.encode(base.key), freshClone: isFreshClone }, 'repo opened')
  ...
  return {
    ...
    /** RPC-friendly read of the secrets-key-version counter. */
    async getSecretsKeyVersion () {
      await base.update()
      const entry = await secretsView.get('secrets-key-version')
      return entry ? entry.value : 0
    },

    /** RPC-friendly read of a per-recipient sealed-box envelope. */
    async getSecretsKeyEnvelope (recipientPubKeyHex) {
      await base.update()
      const entry = await secretsView.get('secrets-key/' + recipientPubKeyHex)
      if (!entry) return null
      return entry.value // { encryptedKey (hex), keyVersion }
    },

    /** Returns the encrypted payload stored for `path`, or null. */
    async getSecretFile (path) {
      await base.update()
      const entry = await secretFilesView.get(path)
      return entry ? entry.value : null
    },

    /** Returns a list of all stored secret paths. */
    async listSecretFiles () {
      await base.update()
      const out = []
      for await (const entry of secretFilesView.createReadStream()) {
        out.push(entry.key)
      }
      return out
    },
    ...
    /**
     * Returns a snapshot of all valid git refs as [{ key, value }, ...].
     * Used by the remote-helper list handler and the RPC proxy so callers
     * don't need direct access to the bee-backed view.
     */
    async listRefs () {
      await base.update()
      const out = []
      for await (const entry of base.view.createReadStream()) {
        if (entry.key.startsWith('refs/') || entry.key === 'HEAD') {
    ...
    /**
     * RPC-friendly status snapshot. Returns primitives so the repo-rpc
     * transport doesn't need to serialise Autobase internals. Pass the
     * seeder's swarm manager so peer counts can be included.
     */
    async getStatusSnapshot ({ swarm = null } = {}) {
      await base.update()
      const writers = []
      for await (const entry of writersView.createReadStream()) {
        writers.push({ key: entry.key, indexer: !!entry.value?.indexer })
      }
      let rejectedPushes = 0
      try {
        for await (const _entry of base.view.sub('rejections').createReadStream()) { // eslint-disable-line no-unused-vars
          rejectedPushes++
        }
      } catch {}
      const secrets = []
      try {
        for await (const entry of secretsView.createReadStream()) {
          if (typeof entry.key === 'string' && entry.key.startsWith('secrets-key/')) continue
          if (entry.key === 'secrets-key-version') continue
          secrets.push(entry.key)
        }
      } catch {}
      let peers = 0
      if (swarm && typeof swarm.connectedPeers === 'function') {
        try { peers = swarm.connectedPeers(base.key) } catch {}
      }
      return {
        repoKey: base.key.toString('hex'),
        peers,
        signedLength: base.signedLength,
        pendingOps: base.pendingLength,
        writers: writers.length,
        indexers: writers.filter(w => w.indexer).length,
        rejectedPushes,
        secrets
      }
    },
