...
async function applySecretsKeyEnvelope (op, writerKey, host, secretsView) {
  ...
  // Accept if this is the next version (currentVersion + 1)
  if (op.keyVersion !== currentVersion + 1) {
    log.warn({ keyVersion: op.keyVersion, currentVersion }, 'secrets-key-envelope: wrong keyVersion, dropping')
    return
  }
  ...
  // Track the current key version so rotate can reference it
...
export async function openRepo (corestore, identity, opts = {}) {
  const base = new Autobase(corestore, opts.key || null, {
    apply,
    open (store) {
